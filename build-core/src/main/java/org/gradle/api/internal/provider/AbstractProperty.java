...
public abstract class AbstractProperty<T, S extends ValueSupplier> extends AbstractMinimalProvider<T> implements PropertyInternal<T> {
    ...
    public void attachProducer(ModelObject owner) {
        if (this.producer == null) {
            ...
            throw new IllegalStateException(String.format("%s is already declared as an output property of %s. Cannot also declare it as an output property of %s.", getDisplayName().getCapitalizedDisplayName(), this.producer, owner));
        }
    }
    ...
    private Task getProducerTask() {
        ...
        if (task == null) {
            ...
            throw new IllegalStateException(String.format("%s is declared as an output property of %s but does not have a task associated with it.",getDisplayName().getCapitalizedDisplayName(),producer));
        }
        return task;
    }
    ...
    private static class NonFinalizedValue<S> extends FinalizationState<S> {
        ...
        private String cannot(String what, DisplayName displayName, String reason) {
            ...
            return String.format("Cannot %s the value of %s because %s.", what, displayName.getDisplayName(), reason);
        }
    }
